...
downstream reasoning on GPQA Diamond and MMLU-Pro benchmarks.
...
# ════════════════════════════════════════════════════════════════════════════════
# Phase B: Downstream Evaluation (GPQA Diamond + MMLU-Pro)
# ════════════════════════════════════════════════════════════════════════════════
...
def prepare_gpqa_diamond(tokenizer):
    """Loads and formats the GPQA Diamond dataset for MCQ evaluation."""
    print("Loading GPQA Diamond dataset...")
    try:
        ds = load_dataset("Idavidrein/gpqa", "gpqa_diamond", split="train", trust_remote_code=True)
    except Exception as e:
        print(f"  Trying alternative GPQA source... ({e})")
        try:
            ds = load_dataset("openai/gpqa", "gpqa_diamond", split="train")
        except Exception as e2:
            print(f"  GPQA unavailable ({e2}). Using synthetic fallback.")
            return _gpqa_synthetic_fallback(tokenizer)
    ...
    for item in ds:
        ...
        formatted.append({"prompt": prompt, "correct_idx": correct_idx, "num_choices": 4})

    print(f"  Loaded {len(formatted)} GPQA Diamond questions.")
    return formatted


def _gpqa_synthetic_fallback(tokenizer):
    """Minimal synthetic fallback if GPQA is unavailable."""
    questions = [
        {"q": "What is the structural geometry of methane (CH4)?",
         "a": "Tetrahedral", "wrong": ["Linear", "Trigonal planar", "Octahedral"]},
        {"q": "Which thermodynamic law states entropy always increases in isolation?",
         "a": "Second Law", "wrong": ["First Law", "Third Law", "Zeroth Law"]},
        {"q": "What molecule is the primary energy currency in cells?",
         "a": "ATP", "wrong": ["DNA", "Glucose", "NADH"]},
    ]
    formatted = []
    for item in questions:
        choices = sorted([item["a"]] + item["wrong"])
        correct_idx = choices.index(item["a"])
        prompt = f"Question: {item['q']}\nA) {choices[0]}\nB) {choices[1]}\nC) {choices[2]}\nD) {choices[3]}\nAnswer:"
        formatted.append({"prompt": prompt, "correct_idx": correct_idx, "num_choices": 4})
    return formatted * 15  # 45 samples
...
def run_evaluations(llm, tokenizer, device, train_results):
    """
    Evaluates baseline, FlatSAE, and HyperSAE hooks on GPQA Diamond
    and MMLU-Pro benchmarks.
    """
    print("\n" + "═" * 72)
    ...
    gpqa_data = prepare_gpqa_diamond(tokenizer)
    ...
    print("  [Baseline]")
    gpqa_baseline = evaluate_mcq_benchmark(llm, tokenizer, device, gpqa_data, choice_tokens_4)
    print(f"  Baseline Accuracy: {gpqa_baseline:.2f}%")

    print("  [FlatSAE]")
    gpqa_flat = evaluate_mcq_benchmark(llm, tokenizer, device, gpqa_data, choice_tokens_4, hook_fn=flat_hook)
    print(f"  FlatSAE Accuracy:  {gpqa_flat:.2f}%")

    print("  [HyperSAE]")
    gpqa_hyper = evaluate_mcq_benchmark(llm, tokenizer, device, gpqa_data, choice_tokens_4, hook_fn=hyper_hook)
    print(f"  HyperSAE Accuracy: {gpqa_hyper:.2f}%")

    eval_results["gpqa_diamond"] = {
        "baseline": gpqa_baseline,
        "flatsae": gpqa_flat,
        "hypersae": gpqa_hyper,
        "num_questions": len(gpqa_data)
    }
